...
io.on("connection", socket => {
  ...
  socket.on("2_to_4_send-req-to-driver", msg => {
    ...
    // @driverWithDistance: mảng driver READY cùng Harversine của nó tới khách
    var driverWithDistance = drivers;
    for (let i = 0; i < driverWithDistance.length; i++) {
      driverWithDistance[i]["distance"] = distances[i];
    }
    // @sortedDrivers là mảng @driverWithDistance xếp lại từ gần nhất đến xa nhất
    ...
    /* @attemp: N lần tìm:
     * @config.SEND_REQUEST_ATTEMPT: N tìm tối đa
     * nếu số driver READY nhỏ hơn SEND_REQUEST_ATTEMPT thì số lần tìm driver cho request này là số driver READY,
     * ngược lại thì tìm tối đa SEND_REQUEST_ATTEMPT lần
     */
    var attempt =
      config.SEND_REQUEST_ATTEMPT < sortedDrivers.length
        ? config.SEND_REQUEST_ATTEMPT
        : sortedDrivers.length;
    // for ( let driver in sortedDrivers){
    if (attempt > 0) {
      let driver = sortedDrivers[0]; // chọn driver ngắn nhất
      ...
      let index = 1; // xét N - 1 driver còn lại, tức index = 1
      var handleInterval = setInterval(() => {
        console.log(`Attempt -> [${attempt}]`);
        // quá N lần hoặc req đã đc accept
        if (--attempt < 1 || isRequestAccepted) {
          clearInterval(handleInterval);
          isRequestAccepted = false;
          handleInterval = 0;
        } else {
          // nếu chưa quá N lần và req chưa đc accept
          driver = sortedDrivers[index++]; // xét driver gần nhất tiếp theo
          const driverId = driver.driverId;
          // gửi req
          io.sockets.emit(
            "2_to_4_send-req-to-driver",
      ...
      }, config.REQUEST_DRIVER_RESPONSE_TIME); // @config.REQUEST_DRIVER_RESPONSE_TIME: khoảng cách mỗi lượt tìm driver
    } else {
